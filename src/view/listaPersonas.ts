  ...
  export class VerPersonas {
    ...
    var buttonContainer = container
      ...
      filas
        .append("td")
        .text((d) =>
          d.fechaN ? new Date(d.fechaN).toLocaleString().split("T")[0] : ""
        );
      filas
        .append("td")
        .append("button")
    ...
    private actualizarTabla() {
      ...
      var filasOld = tbody
          .selectAll("tr")
          .data(DataActuzalizado, (d:any) => d.id);
    ...
    private formulario(
      container_form: d3.Selection<d3.BaseType, unknown, HTMLElement, any>
    ) {
      ...
      buttonContainer
        ...
        .on("click", () => {
          ...
          if (nombre && apellidos && edad > 0 && correo && telefono && fechaN) {
            var id =
              this.PersonasMap.size > 0
                ? Math.max(...Array.from(this.PersonasMap.keys())) + 1
                : 1;

            const confirmar = window.confirm(
              "¿Estás seguro de que deseas registrar esta persona?"
            );
            if (!confirmar) {
            ...
            alert(
              "Todos los campos son obligatorios y la edad debe ser válida."
            );
          }
        });
      ...
      buttonContainer
        ...
        .on("click", () => {
          ...
          var confirmar = window.confirm(
            "¿Estás seguro de que deseas actualizar los datos de esta persona?"
          );
          if (!confirmar) {
            this.ventana.ocultar();
